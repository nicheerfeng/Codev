# codev-shell-integration (PowerShell)
...
if ($global:__CODEV_HOOKS_LOADED) { return }
$global:__CODEV_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__codev_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__codev_urlencode {
    param([string]$s)
...
function global:__codev_install_readline {
    if ($global:__codev_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__codev_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__codev_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __codev_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__codev_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__codev_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($loc.Provider.Name -eq 'FileSystem') {
        ...
        $cwdEnc = __codev_urlencode $cwd
        $hostName = [System.Environment]::MachineName
        $osc7 = "$esc]7;file://$hostName$cwdEnc$esc\"
    }

    $original = if (Test-Path Function:__codev_user_prompt) {
        try { & __codev_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
