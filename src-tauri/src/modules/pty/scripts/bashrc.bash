# codev-shell-integration (bashrc)
#
...
if [ -z "$__CODEV_HOOKS_LOADED" ]; then
  ...
  _codev_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_codev_urlencode "$PWD")"
    if [ -z "$__CODEV_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __CODEV_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_codev_precmd:"*) ;;
    *) PROMPT_COMMAND="_codev_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
