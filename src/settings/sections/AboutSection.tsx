...
export function AboutSection() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <SectionHeader title={translate("About")} description="" />
      ...
      <div className="flex items-center gap-4 rounded-xl border border-border/60 bg-card/60 p-5">
        ...
        <div className="flex min-w-0 flex-col">
          ...
          <span className="text-[11px] text-muted-foreground">
            {translate("Open-source terminal emulator")}
          </span>
      ...
      <dl className="grid grid-cols-[110px_1fr] gap-y-2.5 text-[12px]">
        <dt className="text-muted-foreground">{translate("Build")}</dt>
        <dd className="font-mono text-[11.5px]">
          {build ? `${build} · v${version}` : `v${version}`}
        </dd>

        <dt className="text-muted-foreground">{translate("Bundle ID")}</dt>
        <dd className="font-mono text-[11.5px]">app.crynta.terax</dd>

        <dt className="text-muted-foreground">{translate("License")}</dt>
        <dd>Apache 2.0</dd>

        <dt className="text-muted-foreground">{translate("Source code")}</dt>
        <dd>
          <button
            type="button"
        ...
        <dt className="text-muted-foreground">{translate("Website")}</dt>
        <dd>
          <button
            type="button"
