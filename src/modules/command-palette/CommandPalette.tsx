...
export function CommandPalette({
  ...
  const placeholder = inThemes
    ...
    : parsed.mode === "content"
      ...
      : parsed.mode === "history"
        ? t("Search command history...")
        : t("Type a command, > for history, # to find in files");
  ...
  return (
    <CommandDialog
      ...
      description={t("Run a command, switch theme, or search your workspace.")}
          ...
          <CommandList className="max-h-none overflow-visible pr-3">
            {inThemes ? (
              <CommandGroup heading={t("Themes")}>
                ...
                {themes.length === 0 ? <StatusItem label={t("No themes")} /> : null}
              </CommandGroup>
              ...
              <CommandGroup heading={t("Command history")}>
                {!insertCommand ? (
                  <StatusItem label={t("Open a terminal to run history")} />
                ) : (
                  <AsyncBody
                    loading={history.loading}
                    error={history.error}
                    empty={history.results.length === 0}
                    emptyLabel={t("No history")}
                    t={t}
              ...
              <CommandGroup heading={t("Search modes")}>
                {MODE_HINTS.map((hint) => (
                  <CommandItem
                    key={hint.sigil}
...
function AsyncBody({
  ...
  if (error) {
    return (
      <>
        <StatusItem label={t("Search failed")} tone="error" />
...
function EmptyHint({ t }: { t: (text: string) => string }) {
  return (
    <div className="flex flex-col items-center gap-2 px-4 py-10 text-center text-sm text-muted-foreground">
      <HugeiconsIcon icon={CommandIcon} size={18} strokeWidth={1.5} />
      <span>{t("No commands found. Type ? to see search modes.")}</span>
    </div>
  );
}
