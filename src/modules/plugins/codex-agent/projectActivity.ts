...
type CodexViewStatus = "starting" | "running" | "stopping" | "failed" | "idle";

export type ActivityThread = {
  cwd: string;
  status?: CodexViewStatus;
  waiting?: boolean;
  name?: string | null;
  summary?: string | null;
};
...
export function shouldSkipFinishNotification(input: {
  ...
  return (
    input.visible && input.focused && !input.minimized && input.codexActive
  );
}
...
export function finishNotificationCopy(input: {
  ...
  const project = input.name.trim() || "Codex";
  ...
  const excerpt = summary
    ? summary.length > 140
      ...
      : summary
    : "";
  const title = `Codex · ${input.count === 1 && thread ? thread : project}`;
  if (input.failed)
    return {
      title,
